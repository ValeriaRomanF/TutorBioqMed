...
- repassLinkUrl: Obligatoriamente provee un enlace real, activo, verificado y de excelente calidad académica para repasar el concepto.
- repassLinkLabel: Una etiqueta legible y profesional para el enlace, ej. "PubMed - Cetoacidosis Diabética" o "MedlinePlus - Intolerancia a la Lactosa".
7. Proporciona un "Dato Curioso" científico de alto valor basado en la evidencia sobre el tema, con una referencia académica formal (autor/organización, año) y un enlace web oficial, verificado y funcional (OMS, PubMed, etc.) para que exploren más.
...
- Para garantizar que el enlace SIEMPRE exista y esté activo, utiliza preferentemente URLs de búsquedas directas o índices temáticos estables de sitios confiables. Sigue estos formatos exactos:
  - Buscador de PubMed con término de búsqueda exacto (en inglés): 'https://pubmed.ncbi.nlm.nih.gov/?term=[tema_relevante]' (ej. 'https://pubmed.ncbi.nlm.nih.gov/?term=gout+metabolism' o 'https://pubmed.ncbi.nlm.nih.gov/?term=lactose+intolerance+lactase').
  - Buscador oficial de MedlinePlus en español: 'https://find.medlineplus.gov/search?query=[tema_relevante]' o 'https://medlineplus.gov/spanish/[nombre_tema].html' (ej. 'https://find.medlineplus.gov/search?query=deshidratacion+isotonica' o 'https://medlineplus.gov/spanish/diabetes.html').
...
- Realiza una autoverificación mental antes de generar el JSON para asegurar que la URL sea sintácticamente válida, apunte al dominio legítimo, y que el parámetro de búsqueda sea pertinente para el tema evaluado.`;

    const promptText = `Genera un caso clínico único e interactivo de bioquímica médica para el tema: "${theme}". ${customPrompt ? `Detalles adicionales solicitados: ${customPrompt}` : ""}`;
